...
import io.github.jhipster.sample.web.rest.UserResource;
...
public class UserService {
    ...
    public User createUser(String login, String password, String firstName, String lastName, String email,
        ...
        UserDTO currentUserDTO = ResponseUtil.wrapOrNotFound(getUserWithAuthoritiesByLogin(newUser.getLogin()).map(UserDTO::new )).getBody();
        //UserDTO currentUserDTO = userResource.getUser(newUser.getLogin()).getBody();
         User currentUser = userMapper.userDTOToUser(currentUserDTO);
        UserDashboard userDashboard = new UserDashboard();
        userDashboard = userDashboard.user(currentUser);
        userDashboard.setId(currentUser.getId());
        log.error("V2***BEFORE_SAVE_TO_REPO------Created Information for UserDashoard: {}", userDashboard);
        log.error("V2***BEFORE_SAVE_TO_REPO------Created Information for UserDashoard - UserId: {}", userDashboard.getUser().getId());
        log.error("V2***BEFORE_SAVE_TO_REPO------   -> Information for User: {}", currentUser);
        log.error("V2***BEFORE_SAVE_TO_REPO------   -> Information for UserId: {}", currentUser.getId());
        userDashboardRepository.save(userDashboard);
        userDashboardSearchRepository.save(userDashboard);
        log.error("V2***AFTER_SAVE_TO_REPO-------SAVED Information for UserDashoard: {}", userDashboard);
    ...
    public User createUser(UserDTO userDTO) {
        ...
        userSearchRepository.save(user);
        log.debug("DTO-Created Information for User: {}", user);
        return user;
    }
